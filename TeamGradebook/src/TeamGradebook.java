...
public class TeamGradebook {
	...
	private String handleGetNames() {
		...
		for(int i=0; i<students.size(); i++){
			allnames= allnames+students.get(i).getName()+" ";
		}
	...
	private String handleAddTeam(String teamName, ArrayList<String> memberNames) {
		...
		for(int i=0; i<memberNames.size(); i++){
			String studentName=memberNames.get(i);
			if(this.getStudentByName(studentName)==null){
				Student virtual= new Student(studentName);
				students.add(virtual);
				teamMembers.add(virtual);
			}else{
				teamMembers.add(this.getStudentByName(studentName));
			}
			
		}
		Team createdTeam = new Team(teamName,teamMembers);
	...
	private String handleGetAverage(String studentName) {
		...
		for(int i=0; i<students.size(); i++){
			if (students.get(i).getName().equals(studentName)){
				varusScore=students.get(i).personaverage();
			}
		}
		//TODO: Your code here
		return Long.toString(Math.round(varusScore));
	}
	...
	private String handleGetBestTeam() {
		...
		String name="";
		for(int i=0; i<teams.size(); i++){
			if (teams.get(i).getAverageTeamScore()>first){
				first=teams.get(i).getAverageTeamScore();
				name=teams.get(i).getTeamName();
			}
		}
		//TODO: Your code here
		return name;
