...
public class MapProblems {
	 ...
	 * 
	 * This can be done with for loops, but use a map
	 * 
	 * {1,2,3} yields false
	 * {1,2,1} yields true
	 * @param input 
	...
	public static boolean hasDuplicates(int[] input) {
		HashMap<Integer, Integer> myVar= new HashMap<Integer, Integer>();
		for(int i=0; i<input.length; i++){
			myVar.put(input[i],i);
		}
		if (myVar.size()<input.length){
			return true;
		}
		return false;
	}
	...
	public static int computeScore(String hand) {
		...
		for(int i=0; i<hand.length(); i++){
			if (myMap.containsKey(hand.charAt(i))==false){
				myMap.put(hand.charAt(i), 1);
			}
			myMap.put(hand.charAt(i), myMap.get(hand.charAt(i))+1);
			
		}
		
        Set<Character> keys=myMap.keySet();
        for(Character key: keys){
        	for(int i=0; i<myMap.get(key); i++){
        		score=score+i;
        		
        	}
        	//score=score+myMap.get(key);
        }
		return score;
	}
	...
	public static HashMap<String,Integer> reverseMap(HashMap<Integer,String> input) {
		Set<Integer> someKey= input.keySet();
		HashMap<String,Integer> myMap= new HashMap<String,Integer>();
		for(Integer oneKey:someKey){
			myMap.put(input.get(oneKey), oneKey);
		}
